...
import { checkAndSyncGithubState } from "@/lib/github";
...
export default async function FeedbackDetailPage({
  ...
  // Check GitHub issue state and sync if changed
  const githubInfo = await checkAndSyncGithubState(feedback.id);

  // Re-read feedback if GitHub sync may have changed its status
  if (githubInfo) {
    const refreshed = await prisma.feedback.findUnique({
      where: { id: feedback.id },
      select: { status: true },
    });
    if (refreshed && refreshed.status !== feedback.status) {
      feedback.status = refreshed.status;
    }
  }
    ...
    <div className="p-8 max-w-5xl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                {feedback.githubIssueUrl && (
                  <div className="space-y-2">
                    <div className="flex items-center gap-2">
                      ...
                      >
                        {githubInfo
                          ? `#${githubInfo.number}: ${githubInfo.title}`
                          : "View GitHub Issue"}
                      </a>
                      {githubInfo && (
                        <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                          githubInfo.state === "closed"
        ...
        <div className="space-y-6">
          {feedback.screenshot ? (
            <ScreenshotViewer screenshot={feedback.screenshot} />
          ) : (
