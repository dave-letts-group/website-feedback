"use strict";var WebFeedback=(()=>{var p=Object.defineProperty;var b=Object.getOwnPropertyDescriptor;var m=Object.getOwnPropertyNames;var v=Object.prototype.hasOwnProperty;var x=(s,t)=>{for(var e in t)p(s,e,{get:t[e],enumerable:!0})},w=(s,t,e,r)=>{if(t&&typeof t=="object"||typeof t=="function")for(let a of m(t))!v.call(s,a)&&a!==e&&p(s,a,{get:()=>t[a],enumerable:!(r=b(t,a))||r.enumerable});return s};var _=s=>w(p({},"__esModule",{value:!0}),s);var E={};x(E,{FeedbackWidget:()=>d});var y="https://cdn.jsdelivr.net/npm/html2canvas-pro@1.6.7/dist/html2canvas-pro.min.js";function k(){return new Promise((s,t)=>{if(window.html2canvas)return s(window.html2canvas);let e=document.createElement("script");e.src=y,e.onload=()=>{window.html2canvas?s(window.html2canvas):t(new Error("html2canvas loaded but not available on window"))},e.onerror=()=>t(new Error("Failed to load html2canvas")),document.head.appendChild(e)})}function u(s){return s.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function S(s){return u(s).replace(/"/g,"&quot;")}function F(s){let t=s.trim();return/^(https?:|mailto:|tel:|\/|#|\.)/i.test(t)?t:null}var h={"bottom-right":{bottom:"calc(24px + env(safe-area-inset-bottom, 0px))",right:"24px"},"bottom-left":{bottom:"calc(24px + env(safe-area-inset-bottom, 0px))",left:"24px"},"bottom-center":{bottom:"calc(24px + env(safe-area-inset-bottom, 0px))",left:"50%",transform:"translateX(-50%)"},"top-right":{top:"calc(24px + env(safe-area-inset-top, 0px))",right:"24px"},"top-left":{top:"calc(24px + env(safe-area-inset-top, 0px))",left:"24px"},"top-center":{top:"calc(24px + env(safe-area-inset-top, 0px))",left:"50%",transform:"translateX(-50%)"},"middle-right":{top:"50%",right:"24px",transform:"translateY(-50%)"},"middle-left":{top:"50%",left:"24px",transform:"translateY(-50%)"}},d=class extends HTMLElement{_shadow;_isOpen=!1;_screenshot=null;_captureState="idle";_captureInFlight=!1;_category="general";_rating=0;_submitting=!1;_submitted=!1;static get observedAttributes(){return["position","site-key","api-url","page-title","page-id","user-id","user-name","metadata","theme-color","hide-trigger","footer-text","footer-link","capture-enabled","capture-target","capture-scale","capture-timeout-ms","capture-format","capture-quality"]}constructor(){super(),this._shadow=this.attachShadow({mode:"open"})}connectedCallback(){this.render()}attributeChangedCallback(){this._shadow.innerHTML&&this.render()}open(){this._openModal()}close(){this._closeModal()}async captureNow(){return await this._runCapture(),this._screenshot}get _captureEnabled(){let t=this.getAttribute("capture-enabled");return t===null?!0:t!=="false"&&t!=="0"}get _captureTarget(){return this.getAttribute("capture-target")||"viewport"}get _captureScale(){let t=parseFloat(this.getAttribute("capture-scale")||"");return isFinite(t)&&t>0?t:.35}get _captureTimeoutMs(){let t=parseInt(this.getAttribute("capture-timeout-ms")||"",10);return isFinite(t)&&t>0?t:4500}get _captureFormat(){return this.getAttribute("capture-format")||"jpeg"}get _captureQuality(){let t=parseFloat(this.getAttribute("capture-quality")||"");return isFinite(t)&&t>=0&&t<=1?t:.6}get _themeColor(){return this.getAttribute("theme-color")||"#6366f1"}get _position(){return this.getAttribute("position")||"bottom-right"}_darken(t,e=20){let r=parseInt(t.replace("#",""),16),a=Math.max(0,(r>>16)-e),o=Math.max(0,(r>>8&255)-e),i=Math.max(0,(r&255)-e);return`#${(a<<16|o<<8|i).toString(16).padStart(6,"0")}`}_resolveTarget(){let t=this._captureTarget;if(t==="main"){let e=document.querySelector("main");if(e)return e}return t==="viewport"?document.documentElement:document.body}async _runCapture(){if(this._captureInFlight)return;this._captureInFlight=!0,this._captureState="capturing",this._updateScreenshotUI();let t=this._captureTimeoutMs,e=new Promise((r,a)=>setTimeout(()=>a(new Error("WebFeedback: screenshot capture timed out")),t));try{let r=(async()=>{let a=await k(),o=this._resolveTarget();return(await a(o,{scale:this._captureScale,useCORS:!0,logging:!1,ignoreElements:n=>n===this||n.tagName==="FEEDBACK-WIDGET"||n instanceof HTMLElement&&n.classList.contains("fw-overlay-root")})).toDataURL(`image/${this._captureFormat}`,this._captureFormat==="jpeg"?this._captureQuality:void 0)})();this._screenshot=await Promise.race([r,e]),this._captureState="done"}catch(r){console.warn("WebFeedback: screenshot capture failed",r),this._screenshot=null,this._captureState="failed"}finally{this._captureInFlight=!1}this._updateScreenshotUI()}_updateScreenshotUI(){let t=this._shadow.querySelector(".screenshot-wrap");t&&(this._captureState==="capturing"?t.innerHTML='<div class="screenshot-loading"><div class="spinner"></div><br>Capturing page...</div>':this._captureState==="done"&&this._screenshot?t.innerHTML=`<img src="${this._screenshot}" alt="Screenshot"/>`:this._captureState==="failed"&&(t.innerHTML='<div class="screenshot-unavailable">\u{1F4F7} Screenshot unavailable</div>'))}_openModal(){this._isOpen=!0,this._submitted=!1,this._category="general",this._rating=0,this._screenshot=null,this._captureState=this._captureEnabled?"capturing":"failed",this._captureInFlight=!1,this.render(),this._captureEnabled&&requestAnimationFrame(()=>{this._runCapture()})}_closeModal(){this._isOpen=!1,this._screenshot=null,this._captureState="idle",this._captureInFlight=!1,this._submitted=!1,this.render()}async _submit(){let t=this._shadow.querySelector("#fw-message"),e=t?t.value.trim():"";if(!e){let c=this._shadow.querySelector("#fw-error");c&&(c.textContent="Please enter your feedback",c.style.display="block");return}let r=this.getAttribute("api-url"),a=this.getAttribute("site-key");if(!r||!a){console.error("WebFeedback: api-url and site-key attributes are required");return}this._submitting=!0,this.render();let o=null;try{let c=this.getAttribute("metadata");c&&(o=JSON.parse(c))}catch{}let i=null;try{let c=new URLSearchParams(window.location.search),l={};c.forEach((g,f)=>{l[f]=g}),Object.keys(l).length&&(i=l)}catch{}let n={pageTitle:this.getAttribute("page-title")||document.title,pageId:this.getAttribute("page-id")||null,pageUrl:window.location.href,urlParams:i,userId:this.getAttribute("user-id")||null,userName:this.getAttribute("user-name")||null,message:e,category:this._category,rating:this._rating||null,screenshot:this._screenshot,metadata:o,userAgent:navigator.userAgent};try{if(!(await fetch(`${r.replace(/\/$/,"")}/api/feedback`,{method:"POST",headers:{"Content-Type":"application/json","X-Site-Key":a},body:JSON.stringify(n)})).ok)throw new Error("Submit failed");this._submitting=!1,this._submitted=!0,this.render(),setTimeout(()=>this._closeModal(),2500)}catch(c){console.error("WebFeedback: submit error",c),this._submitting=!1,this.render();let l=this._shadow.querySelector("#fw-error");l&&(l.textContent="Failed to send feedback. Please try again.",l.style.display="block")}}render(){let t=this._themeColor,e=this._darken(t),r=h[this._position]||h["bottom-right"],a=Object.entries(r).map(([o,i])=>`${o}:${i}`).join(";");this._shadow.innerHTML=`
      <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .fw-footer-link {
          color: ${t}; font-weight: 600; text-decoration: none;
        }
          ...
          <div class="modal" id="fw-modal">
            ...
            `:`
              ...
              <div class="modal-body">
                ...
                <div class="stars">
                  ${[1,2,3,4,5].map(o=>`<button class="star ${o<=this._rating?"filled":""}" data-star="${o}">\u2605</button>`).join("")}
                </div>
      ...
      `:""}
    `,this._bindEvents()}_footerHTML(){let t=(this.getAttribute("footer-text")||"").trim(),e=(this.getAttribute("footer-link")||"").trim(),r=e?F(e):null;if(!t&&!r)return"";let a=t||e;return`
      <div part="footer" class="fw-footer">
        <div part="footer-divider" class="fw-footer-divider"></div>
        <p part="footer-text" class="fw-footer-text">${r?`<a part="footer-link" class="fw-footer-link" href="${S(r)}" target="_blank" rel="noopener noreferrer">${u(a)}</a>`:u(a)}</p>
      </div>`}_screenshotSlotHTML(){return!this._captureEnabled||this._captureState==="failed"?'<div class="screenshot-unavailable">\u{1F4F7} Screenshot unavailable</div>':this._captureState==="done"&&this._screenshot?`<img src="${this._screenshot}" alt="Screenshot"/>`:'<div class="screenshot-loading"><div class="spinner"></div><br>Capturing page...</div>'}_bindEvents(){let t=this._shadow.querySelector("#fw-trigger");t&&t.addEventListener("click",()=>this._openModal());let e=this._shadow.querySelector("#fw-overlay");e&&e.addEventListener("click",i=>{i.target===e&&this._closeModal()});let r=this._shadow.querySelector("#fw-close");r&&r.addEventListener("click",()=>this._closeModal()),this._shadow.querySelectorAll(".cat-btn").forEach(i=>{i.addEventListener("click",()=>{this._category=i.dataset.cat||"general",this._shadow.querySelectorAll(".cat-btn").forEach(n=>n.classList.remove("active")),i.classList.add("active")})}),this._shadow.querySelectorAll(".star").forEach(i=>{i.addEventListener("click",()=>{this._rating=parseInt(i.dataset.star||"0"),this._shadow.querySelectorAll(".star").forEach((n,c)=>{n.classList.toggle("filled",c<this._rating)})})});let a=this._shadow.querySelector("#fw-submit");a&&a.addEventListener("click",()=>this._submit());let o=this._shadow.querySelector("#fw-message");o&&o.addEventListener("keydown",i=>{let n=i;n.key==="Enter"&&(n.metaKey||n.ctrlKey)&&this._submit()})}};customElements.get("feedback-widget")||customElements.define("feedback-widget",d);return _(E);})();
